...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 19, 2023 at 06:02 AM
...
('e001', 'b001', 'นาย', 'จิรายุ โอภาสพัฒน์', '1995-07-21', '0992278763', '2010-06-08'),
...
--
DELIMITER $$
CREATE TRIGGER `tg_invoice_insert` BEFORE INSERT ON `invoice` FOR EACH ROW BEGIN
 DECLARE count int DEFAULT 0;
 set count = (select count(*) from invoice);
   SET NEW.invoice_id = CONCAT('rp', LPAD(count+1, 3, '0'));
END
$$
DELIMITER ;
...
--
-- Triggers `repair_detail`
--
DELIMITER $$
CREATE TRIGGER `tg_repair_detail_insert` BEFORE INSERT ON `repair_detail` FOR EACH ROW BEGIN
 DECLARE count int DEFAULT 0;
 set count = (select count(*) from repair_detail);
   SET NEW.repair_id = CONCAT('rp', LPAD(count+1, 3, '0'));
END
$$
DELIMITER ;
...
('tel011', 'กุลนันท์ รัตนาอรุณ', 'teleeeeeeee', 'ddddd');
